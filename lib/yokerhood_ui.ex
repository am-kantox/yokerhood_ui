defmodule YokerhoodUI do
  ...
  repository in phase 5 of its build:
    ...
    * `priv/tailwind/nord_abyss_theme.css` -- daisyUI's own built-in
      `nord` theme (light) paired with a hand-authored `abyss` theme
      (dark) built from the canonical Nord Polar Night/Snow Storm/Frost/
      Aurora hex values directly, so both halves of the pair are
      genuinely Nord (see the moduledoc comment inside that file for why
      daisyUI's own stock `abyss` theme was not enough). Consuming apps
      `@import` it as a relative path from their own `assets/css/app.css`,
      since `deps/yokerhood_ui` is a regular checkout/symlink on disk like
      any other Mix dependency, not a hex package with an asset-bundling
      story of its own.

  Currently consumed by yokerhood.com via a `path:` dependency (this
  repository has a git remote but is not published to Hex) -- see that
  project's README for the exact wiring. `labyrinth` intentionally
